...
def apply_window_background(window, theme_name: str | None = None, preset: str | None = None) -> None:
    ...
    if hasattr(window, 'set_background_image'):
        # Фоновая картинка убрана вместе со страницей оформления:
        # картинки брались из папки themes, а её никто не наполняет.
...
def _brand_accent_rgb(*, is_light: bool) -> tuple[int, int, int]:
    """Акцент бренда как (r, g, b). Единственный источник — branding.py."""
    try:
        from branding import get_palette

        raw = str(get_palette(dark=not is_light).accent).strip().lstrip("#")
...
def _sync_theme_accent_to_qfluent(theme_name: str) -> None:
    ...
    try:
        ...
        # Акцент по умолчанию берётся из branding.py, а не из виндового
        # #0078D4. Пользовательский акцент из «Оформления» выше по приоритету
        # и обрабатывается веткой peek_warmed_accent_color() выше.
        from branding import PALETTE_LIGHT

        qconfig.set(qconfig.themeColor, _QColor(PALETTE_LIGHT.accent))
    except Exception:
        pass
...
def get_theme_tokens(theme_name: str | None = None) -> ThemeTokens:
    ...
    # Запасной акцент — из branding.py под текущую тему, а не виндовый #0078D4.
    accent_rgb = _get_qfluent_themecolor() or _brand_accent_rgb(is_light=is_light)
