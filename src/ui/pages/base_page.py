...
class BasePage(_FluentScrollArea):
    ...
    def __init__(
        ...
        *,
        ...
        #: Лента песчинок внизу страницы. Создаётся лениво, при первом
        #: показе: строить её в конструкторе значило бы платить за фон
        #: на всех сорока страницах, включая те, что человек не откроет.
        ...
        # Показ и скрытие вложенных виджетов (вкладки BlockCheck) меняют
        # то, кто на странице забирает высоту. Qt сообщает об этом
        # LayoutRequest — по нему пересчитываем замыкающее растяжение.
    ...
    def eventFilter(self, watched, event):  # noqa: N802
        try:
            ...
            event_type = None
        if event_type == QEvent.Type.LayoutRequest and watched is getattr(self, "content", None):
            self._sync_trailing_stretch()
            # Лента считает свободное место по низу последнего элемента,
            # а на момент показа страницы раскладка ещё не устоялась и
            # места «не было». Пересчитываем, когда Qt сообщает о
            # перестройке.
            self._layout_particle_layer()
    ...
    def _content_insert_index(self) -> int:
        """Куда класть новый элемент, чтобы растяжение осталось внизу.

        Страницы достраиваются лениво: BlockCheck, например, создаёт
        содержимое вкладки при первом переходе на неё. Если растяжение
        уже добавлено в конец, всё новое уходит под него — и содержимое
        оказывается сдвинутым на высоту экрана.
        """
        if not getattr(self, "_trailing_stretch_added", False):
            return -1
        try:
            return max(0, self.vBoxLayout.count() - 1)
        except Exception:
            return -1
    ...
    def add_widget(self, widget: QWidget, stretch: int = 0):
        ...
        if index < 0:
            ...
            self.vBoxLayout.insertWidget(index, widget, stretch)
        self._sync_trailing_stretch()
    ...
    def add_spacing(self, height: int = 16):
        ...
        if index < 0:
            ...
            self.vBoxLayout.insertItem(index, spacer)
        self._sync_trailing_stretch()
    ...
    def add_section_title(
        ...
        *,
        ...
        if index < 0:
            ...
            self.vBoxLayout.insertWidget(index, label)
        self._sync_trailing_stretch()
        if return_widget:
            return label
    ...
    def _ensure_trailing_stretch(self) -> None:
        """Прижимает содержимое страницы к верху.

        Без замыкающего растяжения QVBoxLayout раздаёт лишнюю высоту
        между элементами. Пока страницы были длинными, это не бросалось
        в глаза, но после чистки разделов пустого места стало больше
        экрана — и заголовок с подзаголовком разъезжались на половину
        окна, особенно в полноэкранном режиме.
        """
        if getattr(self, "_trailing_stretch_added", False):
            return

        layout = getattr(self, "vBoxLayout", None)
        if layout is None:
            return

        try:
            layout.addStretch(1)
            self._trailing_stretch_added = True
            self._trailing_stretch_expands = True
        except Exception:
            return

        self._sync_trailing_stretch()

    def _trailing_stretch_spacer(self):
        """Замыкающее растяжение и его позиция, если оно на месте.

        Ищем по последнему элементу, а не по сохранённой ссылке: PyQt
        отдаёт новую обёртку QLayoutItem на каждый itemAt(), и сравнение
        через `is` здесь не работает. Всё, что добавляют страницы, идёт
        перед растяжением, поэтому последний элемент — это оно.
        """
        layout = getattr(self, "vBoxLayout", None)
        if layout is None or not getattr(self, "_trailing_stretch_added", False):
            return None, -1
        try:
    ...
    def _page_wants_trailing_stretch(self, last_index: int) -> bool:
        """Нужно ли растяжение вообще.

        Если на странице уже есть элемент, который сам забирает высоту —
        список профилей, вкладка BlockCheck, редактор списка, — то
        растяжение не прижимает содержимое к верху, а делит высоту с
        этим элементом пополам. Именно так появлялась пустая нижняя
        половина в «Настройке пресета», «DNS подмене», «Подборе
        стратегии» и «Диагностике»: список получал половину окна, а
        вторую половину съедала пустота.
        """
        layout = getattr(self, "vBoxLayout", None)
        if layout is None:
            return True
        ...
        for index in range(max(0, last_index)):
            ...
            try:
                if widget.isHidden():
                    continue
                if widget.sizePolicy().verticalPolicy() in expanding:
                    return False
            except Exception:
                continue
        return True

    def _sync_trailing_stretch(self) -> None:
        """Включает или гасит замыкающее растяжение под текущий состав."""
        item, index = self._trailing_stretch_spacer()
        if item is None:
            return

        wants = self._page_wants_trailing_stretch(index)
        # Выход по неизменившемуся состоянию обязателен: invalidate()
        # шлёт LayoutRequest, на который мы же и подписаны, и без этой
        # проверки получилась бы бесконечная перекладка.
        if bool(getattr(self, "_trailing_stretch_expands", True)) == bool(wants):
            return
        self._trailing_stretch_expands = bool(wants)
        ...
        try:
            item.changeSize(
                ...
                QSizePolicy.Policy.Expanding if wants else QSizePolicy.Policy.Fixed,
            )
            self.vBoxLayout.setStretch(index, 1 if wants else 0)
    ...
    #: Показывать ли ленту песчинок внизу страниц.
    #:
    #: Выключена по просьбе: «эту полосу частиц снизу надо убрать». Роль
    #: заполнения пустого низа взял на себя слой свечения из
    #: shell/ambient.py — он лежит под всем окном, дышит медленнее и не
    #: притягивает взгляд к нижнему краю.
    #:
    #: Код ленты оставлен на месте, а не удалён: он рабочий, обвешан
    #: тестами и стоил нескольких заходов с замерами. Вернуть — поменять
    #: этот флаг.
    PARTICLES_ENABLED = False

    def _ensure_particle_layer(self):
        """Создаёт слой песчинок. Сбой оформления не роняет страницу."""
        if not self.PARTICLES_ENABLED:
            return None
        if self._particle_layer is not None:
            return self._particle_layer
        try:
            from qfluentwidgets import isDarkTheme

            from shell.particles import ParticleLayer

            # Лента живёт в свободной полосе НИЖЕ содержимого, а не под
            # ним. Это решает сразу две жалобы.
            #
            # «Поверх всего»: под содержимое её было не увести. Порядок
            # слоёв перебран весь — под страницей не видно вовсе, внутри
            # viewport сверху видно поверх карточек, под прозрачным
            # viewport видно, но тогда...
            #
            # ...«лагает, будто пятнадцать кадров»: слой под прозрачным
            # viewport заставлял Qt на каждом кадре перерисовывать всё
            # содержимое над собой — карточки, тексты, поля. Тридцать
            # раз в секунду перерисовывалась вся страница.
            #
            # В свободной полосе перерисовывать нечего и накрывать
            # нечего. Если свободного места нет — ленты просто нет.
            viewport = self.viewport()
            viewport.setAutoFillBackground(False)
            viewport.setStyleSheet("background: transparent;")
            layer = ParticleLayer(self, dark=bool(isDarkTheme()))
            self._particle_layer = layer
        except Exception as exc:
            try:
                from log.log import log as _log

                _log(f"Лента песчинок не создана: {exc}", "DEBUG")
            except Exception:
                pass
            # Пустышка вместо None, иначе попытка будет повторяться на
            # каждом показе страницы.
            self._particle_layer = False
        return self._particle_layer

    def _layout_particle_layer(self) -> None:
        layer = self._particle_layer
        if not layer:
            return
        try:
            ...
            content_bottom = self._content_bottom_in_page()
            free = max(0, available - max(0, content_bottom))
            height = min(BAND_HEIGHT, free)
            if height < MIN_BAND_HEIGHT:
                layer.hide()
                layer.stop()
                return
            layer.setGeometry(0, available - height, self.width(), height)
            # Только на видимой странице. Перестройка раскладки приходит
            # и после скрытия страницы, и без этой проверки лента
            # запускала таймер обратно на невидимой странице.
            if self.isVisible() and not layer.isVisible():
                layer.show()
                layer.start()
        except Exception:
            pass

    def _content_bottom_in_page(self) -> int:
        """Низ последнего настоящего элемента страницы, в её координатах.

        Считать по self.content нельзя: он всегда растянут на всю
        высоту области прокрутки, и «свободного места под содержимым»
        по нему не остаётся никогда — лента не показывалась вовсе.
        Нужен низ последнего видимого виджета в раскладке, а не низ
        контейнера.
        """
        bottom = 0
        try:
            ...
            for index in range(layout.count()):
                ...
                bottom = max(bottom, int(point.y()))
        except Exception:
            return self.height()
        return bottom

    def _start_particles(self) -> None:
        layer = self._ensure_particle_layer()
        if not layer:
            return
        # Показать или спрятать решает _layout_particle_layer: места под
        # ленту может не быть вовсе. Безусловный show() здесь отменял
        # это решение, и слой оставался видимым со старой геометрией.
        #
        # Сам show() при этом обязателен именно там: ребёнок, созданный
        # уже после того, как родитель показан, по умолчанию скрыт —
        # слой существовал, имел верную геометрию и полный набор
        # песчинок, а его paintEvent не вызывался ни разу.
        self._layout_particle_layer()

    def _stop_particles(self) -> None:
        layer = self._particle_layer
        if not layer:
            return
        try:
    ...
    def hideEvent(self, event):  # noqa: N802 (Qt override)
        # Таймер ленты крутится только на видимой странице. Сорок
        # спрятанных страниц с работающими таймерами — это сорок
        # пересчётов по 2600 песчинок в секунду впустую.
