...
"""Страница «О программе» — версия, поддержка, справка."""
...
class AboutPage(BasePage):
    """Страница «О программе» с вкладками: О программе / Справка."""
    ...
    def _switch_tab(self, index: int):
        ...
        # plan.init_kvn всегда False: вкладка удалена.
    ...
    def _retranslate_about_tab(self) -> None:
        try:
            ...
            about_app_name = tr_catalog("page.about.app_name", language=self._ui_language, default="net67 v2 GUI")
            ...
            # Секции «Подписка» и кнопок Premium/KVN больше нет —
            # переводить нечего.
        except Exception:
            pass
    ...
    def update_subscription_status(self, is_premium: bool, days: int | None = None):
        """Больше ничего не отображает.

        Блок подписки удалён из вкладки «О программе», но метод вызывается
        снаружи (ui_state_store, tray), поэтому оставлен как no-op.
        """
        return
    ...
    # Метод _add_motto_block удалён: он нигде не вызывался (вкладка
    # «Справка» строит девиз в about_page_help_build), а внутри жёстко
    # лежал слоган автора исходного проекта.
    ...
    # Раздел «Tab 3: net67 KVN» удалён целиком: вкладка, её сборка и
    # четыре обработчика ссылок на Telegram-канал, бота и GitHub
    # стороннего проекта автора исходников.
