...
#: Открывать окно развёрнутым на весь экран.
#:
#: Приложение внутреннее и рабочее: менеджер открывает его, чтобы
#: посмотреть состояние и нажать одну кнопку, и каждый раз растягивать
#: окно руками — лишнее действие. Сохранённая normal-геометрия при этом
#: не теряется: она применяется, когда окно сворачивают из развёрнутого.
...
class WindowGeometryRuntime:
    ...
    def restore_geometry(self) -> None:
        ...
        try:
            ...
            start_maximized = bool(saved.maximized) or START_MAXIMIZED
            self._last_non_minimized_zoomed = start_maximized
            self._pending_restore_maximized = start_maximized
