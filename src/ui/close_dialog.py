...
class CloseDialog(MessageBoxBase):
    ...
    def _install_accessibility(self, description: str) -> None:
        if self._launch_running:
            ...
            stop_name = "Закрыть net67 и остановить DPI"
            ...
            stop_name = "Закрыть net67 и остановить DPI, недоступно"
        ...
        set_control_accessibility(
            self.titleLabel,
            name=title_name,
            description="Окно выбора действия при закрытии net67.",
        )
        set_control_accessibility(
            self.bodyLabel,
            name=body_name,
        ...
        set_control_accessibility(
            self.trayButton,
            name="Свернуть net67 в трей",
            description="Скрывает окно и оставляет окно доступным из трея. DPI продолжит работать.",
        )
        set_state_text(self.trayButton, "Свернуть net67 в трей")
        set_control_accessibility(
            self.guiOnlyButton,
            name="Закрыть только окно net67",
            description="Закрывает только GUI. DPI продолжит работать в фоне.",
        )
        set_state_text(self.guiOnlyButton, "Закрыть только окно net67")
        set_control_accessibility(
            self.stopDpiButton,
            name=stop_name,
        ...
        set_control_accessibility(
            ...
            name="Отменить закрытие net67",
            description="Закрывает этот вопрос и возвращает вас в программу.",
        )
        set_state_text(self.cancelLinkButton, "Отменить закрытие net67")
