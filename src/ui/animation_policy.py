...
def are_animations_enabled() -> bool:
    """Читает текущее пользовательское состояние переключателя анимаций.

    Непрогретый кэш отдаёт None, и прежнее bool(None) превращало его в
    «выключено». До прогрева настроек анимации оказывались отключены,
    хотя человек их не отключал. Теперь неизвестность трактуется как
    умолчание.
    """
    try:
        from settings.appearance import peek_warmed_animations_enabled

        value = peek_warmed_animations_enabled()
    except Exception:
        value = None
    ...
    if value is None:
        try:
            ...
            return bool(schema.default_appearance()["animations_enabled"])
        except Exception:
            return True
    return bool(value)
