...
TWO_K_SCREEN_WIDTH = 2560
TWO_K_SCREEN_HEIGHT = 1440
TWO_K_WINDOW_WIDTH = 1920
TWO_K_WINDOW_HEIGHT = 1080
MIN_WIDTH = 900
MIN_HEIGHT = 580

# Простой вид показывает одну страницу и два пункта в боковой панели,
# поэтому окно расширенного режима оставляет под ним много пустоты.
SIMPLE_WIDTH = 780
SIMPLE_HEIGHT = 620
SIMPLE_MIN_WIDTH = 700
SIMPLE_MIN_HEIGHT = 520
...
def get_scaled_window_size():
    ...
    if screen_width == TWO_K_SCREEN_WIDTH and screen_height == TWO_K_SCREEN_HEIGHT:
        return TWO_K_WINDOW_WIDTH, TWO_K_WINDOW_HEIGHT
...
def get_window_size_for_mode(advanced: bool) -> tuple[int, int]:
    """Размер окна под режим интерфейса.

    На маленьком экране рассчитанный размер может оказаться меньше
    компактного — тогда компактный не нужен, берём меньшее из двух.
    """
    if advanced:
        return WIDTH, HEIGHT
    return min(SIMPLE_WIDTH, WIDTH), min(SIMPLE_HEIGHT, HEIGHT)


def get_min_size_for_mode(advanced: bool) -> tuple[int, int]:
    """Минимальный размер окна под режим интерфейса."""
    if advanced:
        return MIN_WIDTH, MIN_HEIGHT
    return SIMPLE_MIN_WIDTH, SIMPLE_MIN_HEIGHT
