...
# ВАЖНО: должно совпадать с --name в .github/workflows/windows-release.yml.
# Если разойдётся, приложение соберётся, но упадёт при старте на проверке
# структуры в resolve_application_root().
RUNTIME_EXE_NAME = "net67.exe"
...
class ApplicationPaths:
    ...
    def stable_icon(self) -> Path:
        return self.ico_dir / "net67.ico"

    @property
    def dev_icon(self) -> Path:
        # Отдельной dev-иконки нет — оба канала используют одну.
        return self.ico_dir / "net67.ico"
    ...
    *,
    ...
    if packaged:
        ...
        if runtime_root.name.casefold() != RUNTIME_DIR_NAME.casefold():
            raise RuntimeError(
                "Некорректная структура установленного net67: "
                f"ожидался запуск из папки {RUNTIME_DIR_NAME}, получено {runtime_root}"
            )
        return runtime_root.parent

    # config/runtime_layout.py -> config -> src -> корень проекта
...
def resolve_runtime_root(*, executable: str | Path, packaged: bool) -> Path | None:
    ...
    if runtime_root.name.casefold() != RUNTIME_DIR_NAME.casefold():
        raise RuntimeError(
            "Некорректная структура среды net67: "
