...
TAB_HINTS: dict[str, str] = {
    ...
    # Коротко намеренно. Перечисление четырёх протоколов занимало две
    # строки и ничего не решало: человек всё равно вставляет то, что
    # ему выдали, не сверяясь со списком. Подсказка про подписку
    # полезнее — она не очевидна.
    TAB_LINKS: "Ссылка на сервер или на подписку — из подписки возьмутся все серверы разом.",
}
