...
def apply_winws1_pages_language(
    *,
    ...
    set_button_text_accessibility(
        start_btn,
        tr_catalog("page.winws1_control.button.start", language=language, default="Запустить net67"),
    ...
    if auto_dpi_toggle is not None:
        auto_dpi_toggle.set_texts(
            ...
            tr_catalog("page.winws1_control.setting.autostart.desc", language=language, default="После запуска net67 автоматически запускать текущий DPI-режим"),
        )
    if gui_autostart_toggle is not None:
        gui_autostart_toggle.set_texts(
            tr_catalog("page.control.setting.gui_autostart.title", language=language, default="Автозапуск net67"),
    ...
    if tray_close_mode_combo is not None:
        tray_close_mode_combo.set_texts(
            ...
            tr_catalog("page.control.setting.tray_close_mode.desc", language=language, default="Выберите, когда net67 будет скрывать окно в системный трей"),
        )
