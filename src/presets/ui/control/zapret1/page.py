...
"""net67 v1 mode management page (entry point for zapret1_mode mode)."""
...
class Zapret1ModeControlPage(ControlPageWindowsFeatureMixin, ControlPageActionMixin, BasePage):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            "Управление net67 v1",
            f"Настройка и запуск net67 v1 ({EXE_NAME_WINWS1}). В «Мои пресеты» выбирается пресет, "
    ...
    def _on_top_summary_failed(self, request_id: int, error: str) -> None:
        if request_id != self._refresh_runtime.top_summary_request_id or self._cleanup_in_progress:
            return
        log(f"Не удалось обновить сводку net67 v1: {error}", "WARNING")
