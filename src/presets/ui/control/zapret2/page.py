...
"""net67 v2 mode management page (Strategies landing for zapret2_mode)."""
...
class Zapret2ModeControlPage(ControlPageWindowsFeatureMixin, ControlPageActionMixin, BasePage):
    """Главная страница управления для net67 v2."""
    ...
    def _on_top_summary_failed(self, request_id: int, error: str) -> None:
        ...
        log(f"Не удалось обновить сводку net67 v2: {error}", "WARNING")
    ...
    def _build_ui(self):
        ...
        # Главная кнопка. Стоит первой и единственная: она включает
        # обход, применяет hosts и проверяет доступность — именно она
        # делает WhatsApp и нейросети рабочими.
        #
        # Ниже намеренно НЕ добавляются в раскладку карточка «Статус
        # работы» и строка кнопок «Запустить/Остановить». Раньше на
        # экране жили три источника правды о том, работает ли обход, и
        # они расходились: кнопка предлагала «Включить», а карточка
        # рядом писала «net67 работает». Виджеты создаются и остаются
        # живыми — их обновляет существующая логика страницы, — но
        # спрятаны.
        from PyQt6.QtWidgets import QWidget as _QWidget

        # Склад для виджетов, которые нужны живыми, но не должны
        # появляться на экране. setVisible(False) на самом виджете
        # оказалось недостаточно: карточка всё равно всплыла поверх
        # заголовка страницы. Ребёнок навсегда скрытого контейнера
        # показаться не может в принципе, кто бы ни вызвал ему show().
        self._offscreen_holder = _QWidget(self)
        self._offscreen_holder.setVisible(False)
        self._offscreen_holder.setFixedSize(0, 0)
        ...
        # Не в раскладку: состояние показывает круг наверху. Виджет
        # жив, потому что его обновляет apply_status_plan.
        status_widgets.card.setParent(self._offscreen_holder)
        ...
        management_widgets = build_winws2_pages_management_section(
            ...
            # Круглые кнопки главного экрана. Подкласс нужен потому,
            # что qfluentwidgets ставит значок по minimumSizeHint, а этот
            # метод зовётся из C++: на экземпляре его не подменить.
        ...
        # Тоже не в раскладку: это второй запуск, дублирующий главную
        # кнопку. Виджеты нужны живыми — apply_status_plan переключает
        # их видимость и на этом строит свои решения.
        management_widgets.card.setParent(self._offscreen_holder)
    ...
    def _build_settings_sections(self) -> None:
        ...
        # состояние и автозапуск. Остальное прячется, а не удаляется:
        # на эти виджеты ссылаются переводы, тема и синхронизация настроек.
        from presets.ui.control.simple_view import (
            apply_simple_view,
            attach_advanced_button,
            attach_theme_switch,
        )

        # Минимум настроек внешнего вида — прямо здесь: ради светлой темы
        # ходить в расширенные настройки неудобно.
        attach_theme_switch(self)
        # В простом виде боковой панели нет, и кнопка — единственный вход
        # в полный интерфейс.
        attach_advanced_button(self, self.window())
        apply_simple_view(self)
