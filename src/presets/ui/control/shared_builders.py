...
def build_mode_status_section_common(
    *,
    ...
    if title_text:
        set_state_text(status_title, f"Статус net67: {title_text}")
    if desc_text:
        set_state_text(status_desc, f"Описание состояния net67: {desc_text}")
...
def build_last_status_message_card_common(
    *,
    ...
    if title_text:
        set_state_text(title_label, f"Раздел статуса net67: {title_text}")
    if message_text:
        set_state_text(message_label, f"Последнее сообщение net67: {message_text}")
...
def build_mode_management_section_common(
    *,
    ...
    # Не даём горизонтальной раскладке ужать кнопки уже их текста.
    # В компактном окне надпись обрезалась с обеих сторон и от
    # «Остановить только winws2.exe» оставалось «овить только winw».
    for button in (start_btn, stop_winws_btn, stop_and_exit_btn):
        try:
    ...
    set_control_accessibility(
        progress_bar,
        name="Ход запуска net67: не выполняется",
        description="Показывает, что запуск или остановка net67 выполняется.",
    )
    set_state_text(progress_bar, "Ход запуска net67: не выполняется")
    content_layout.addWidget(progress_bar)

    loading_label = caption_label_cls("")
    loading_label.setVisible(False)
    set_state_text(loading_label, "Статус запуска net67: нет активного запуска")
