...
def build_status_plan(*, state: str | bool, last_error: str, language: str) -> ControlStatusPlan:
    ...
    if phase == "running":
        return ControlStatusPlan(
            phase=phase,
            title=tr_catalog("page.control.status.running", language=language, default="net67 работает"),
    ...
    if phase == "autostart_pending":
        return ControlStatusPlan(
            phase=phase,
            title="Автозапуск net67 запланирован",
    ...
    return ControlStatusPlan(
        phase="stopped",
        title=tr_catalog("page.control.status.stopped", language=language, default="net67 остановлен"),
            ...
            confirmations=(
                ControlConfirmationDialogPlan(
                    ...
                    content=(
                        ...
                        "Что будет сделано:\n"
                        "• В файл hosts будет добавлен отдельный блок net67\n"
        ...
        confirmations=(
            ControlConfirmationDialogPlan(
                ...
                content=(
                    ...
                    "Из hosts будет удалён только блок, созданный net67."
                ),
                revert_checked=True,
            ),
        ),
