...
def set_progress_active_if_changed(progress, active: bool) -> bool:
    ...
    set_state_text(progress, f"Ход запуска net67: {state_text}")
    if next_active:
        progress.start()
    else:
        progress.stop()
...
def set_loading_status_accessibility(label, *, active: bool, text: object) -> None:
    ...
    state = f"Статус запуска net67: {value}" if value else "Статус запуска net67: нет активного запуска"
...
def apply_status_plan(
    ...
    *,
    ...
    set_state_text(status_title, f"Статус net67: {plan.title}")
    set_accessible_description(status_title, plan.description)
    set_state_text(status_desc, f"Описание состояния net67: {plan.description}")
    set_state_text(status_dot, f"Индикатор состояния net67: {plan.title}")
...
def apply_last_status_message(
    ...
    *,
    ...
    set_state_text(message_label, f"Последнее сообщение net67: {text}")
