...
class ControlTopSummaryWidget(QWidget):
    ...
    def __init__(self, *, language: str, mode_value: str, initial_icon_delay_ms: int = 0, parent=None):
        ...
        # Плашка подписки убрана из продукта. Виджет создаётся, но не
        # добавляется в layout и скрыт: на premium_item ещё ссылаются
        # retranslate() и presets/ui/control/*/page.py, поэтому убирать
        # атрибут целиком нельзя.
        ...
        self.premium_item.setVisible(False)
        ...
        for item in (self.profiles_item, self.mode_item):
