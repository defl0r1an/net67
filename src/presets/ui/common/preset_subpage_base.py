...
def _set_runtime_toggle_accessibility(button, plan: RuntimeToggleButtonPlan, *, runtime_available: bool) -> None:
    ...
    if runtime_available:
        description = (
            "Останавливает запущенный net67 с этим пресетом."
            if plan.should_stop
            else "Запускает net67 с открытым пресетом."
        )
    else:
        description = "Управление net67 сейчас недоступно."
    set_control_accessibility(
        button,
        name=name,
...
class PresetRawEditorPage(BasePage):
    ...
    def _toggle_runtime(self) -> None:
        ...
        if runtime is None:
            self._show_error("Управление net67 сейчас недоступно.")
            return
        self._commit_pending_content_change()
        try:
