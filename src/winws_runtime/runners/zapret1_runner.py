...
"""
Strategy runner for net67 v1 (winws.exe).
...
class Winws1StrategyRunner(StrategyRunnerBase):
    ...
    def _compile_preset_artifact(self, preset_path: str) -> PreparedPresetArtifact:
        ...
        for _attempt in range(2):
            ...
            artifact = PreparedPresetArtifact(
                ...
                launch_args=(at_config_launch_arg(at_config_path, self.work_dir),),
                validation_ok=True,
                validation_report="",
            )
    ...
    def _write_winws1_dry_run_at_config(self, artifact: PreparedPresetArtifact) -> str:
        ...
        if not launch_arg.startswith("@") or len(launch_arg) <= 1:
            raise ValueError("net67 v1 dry-run requires an @config launch artifact")

        # Аргумент теперь относительный — он рассчитан на cwd=work_dir у
        # winws, а не на текущий каталог процесса приложения. Открывать
        # его напрямую нельзя: у GUI cwd совсем другой, и dry-run падал
        # бы с «файл не найден» на ровном месте.
        source_config_path = launch_arg[1:]
        if not os.path.isabs(source_config_path):
            source_config_path = os.path.join(str(self.work_dir or ""), source_config_path)
        ...
        *,
        ...
        try:
            ...
            completed = subprocess.run(
                [self.winws_exe, at_config_launch_arg(dry_run_config_path, self.work_dir)],
    ...
    def _launch_args_files_exist(self, launch_args: tuple[str, ...]) -> bool:
        # Аргумент относительный и разрешается от work_dir, а не от cwd
        # процесса приложения: иначе проверка всегда говорила бы «файла
        # нет» и кэш готового пресета пересобирался бы на каждый запуск.
        return at_config_files_exist(launch_args, self.work_dir)
    ...
    def start_from_preset_file(
        ...
        """
        Запускает движок net67 v1 из выбранного preset-файла.
