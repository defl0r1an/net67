...
"""
Strategy runner for net67 v2 (winws2.exe).
...
class Winws2StrategyRunner(StrategyRunnerBase):
    """
    Runner for net67 v2 (winws2.exe).
    ...
    def _run_preset_dry_run_locked(
        ...
        *,
        ...
        # Аргументы и cwd в лог обязательно. Когда winws2 не понимает путь
        # к @config, он печатает только свой баннер версии — по такому
        # сообщению причину не найти, а разница между машинами была
        # ровно в пути. Один раз это уже стоило долгих поисков.
        log(
            f"Preset dry-run failed before winws2 start (code: {self._last_spawn_exit_code}); "
            f"args={dry_run_artifact.launch_args}; cwd={self.work_dir}"
    ...
    def _log_winws2_launch_command(self, *, cmd: list[str], artifact: PreparedPresetArtifact) -> None:
        ...
        for arg in artifact.launch_args:
            ...
            config_path = resolve_at_config_arg(value, self.work_dir)
            try:
    ...
    def start_from_preset_file(
        ...
        """
        Запускает движок net67 v2 из выбранного preset-файла.
