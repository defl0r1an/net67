...
CONFLICTING_PROCESSES = {
    "ProcessHacker.exe": {
        ...
        "solution": "Закройте Process Hacker и повторите запуск net67",
    },
    "procexp.exe": {
        "name": "Process Explorer",
        "reason": "Может конфликтовать с WinDivert",
        "solution": "Закройте Process Explorer и повторите запуск net67",
    },
    "procexp64.exe": {
...
def build_launch_conflict_advice() -> tuple[str, str] | None:
    """Возвращает подсказку только после неудачного запуска net67."""
    ...
    cause = f"{names}, похоже, помешал запуску net67: WinDivert не смог открыться"
    solution = "\n".join(solutions) or "Закройте конфликтующую программу и повторите запуск net67"
...
def _build_dynamic_conflict_advice() -> tuple[str, str] | None:
    ...
    if holders:
        ...
        return (
            ...
            "Закройте эту программу и повторите запуск net67",
        )
    ...
    if foreign_paths:
        return (
            f"Драйвер WinDivert установлен другой программой: {foreign_paths[0]}",
            "Закройте ту программу (или удалите её драйвер) и повторите запуск net67",
        )
...
def get_conflicting_processes_report() -> str:
    ...
    lines.append("Закройте эти программы, если запуск net67 завершился ошибкой.")
    return "\n".join(lines)
