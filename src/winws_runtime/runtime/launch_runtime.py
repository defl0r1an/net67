...
class PresetLaunchRuntime(QObject):
    """Координатор запуска, остановки и переключения DPI.

    Наследование от QObject обязательно, и вот почему.

    Обработчики завершения (_on_dpi_start_finished, _on_dpi_stop_finished)
    подключаются к сигналам рабочих потоков. Если приёмник не QObject, Qt
    не может определить его поток и делает соединение прямым — слот
    выполняется в том потоке, который испустил сигнал.

    Пока запуск шёл из UI-потока, это сходило с рук. Но кнопка «Включить»
    вызывает start() из потока оркестратора, и там прямое соединение
    приводило к тому, что обработчик просто не отрабатывал: в логе после
    «Пресет успешно запущен» обрывалась вся цепочка, set_busy(False) не
    вызывался, и на странице навсегда оставалось «Запуск net67...» с
    заблокированными кнопками.

    С QObject соединение становится очередным, слот выполняется в потоке
    владельца, и признак занятости снимается как положено.
    """
