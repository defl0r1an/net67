...
# ВНИМАНИЕ. Здесь стоял репозиторий автора исходного проекта. Если его
# оставить, автообновление скачает оригинальный net67 и перезапишет им
# net67 — со всем брендингом, донатом и подпиской обратно.
#
# Пустая строка = резервный источник обновлений через GitHub отключён.
#
# Наш репозиторий с выпусками. Приложение смотрит на последний релиз и
# берёт из него .exe установщика.
#
# Оговорка для России: api.github.com оттуда доступен неровно.
# Механизм это переживает — проверка молча не находит обновлений и
# ничего не ломает, — но рассчитывать, что все двадцать менеджеров
# обновятся сами, не стоит.
GITHUB_REPO = "defl0r1an/net67"
