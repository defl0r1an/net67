...
class ConnectionTestWorker(QObject):
    ...
    def check_dns_poisoning(self):
        ...
        try:
            ...
            if results['summary']['dns_poisoning_detected']:
                ...
                self.log_message("")
                self.log_message("2. После смены DNS перезапустите net67")
    ...
    def interpret_youtube_results(self):
        ...
        if ssl_problems:
            ...
            self.log_message("   1. ✅ Убедитесь что net67 запущен")
            self.log_message("   2. ✅ Проверьте что выбрана рабочая стратегия")
            self.log_message("   3. ✅ Дождитесь полной инициализации net67")
    ...
    def check_zapret_status(self):
        ...
        try:
            ...
            if not winws_found:
                ...
                self.log_message("   net67 не работает!")

        except Exception as e:
            self.log_message(f"❌ Ошибка проверки net67: {e}")
