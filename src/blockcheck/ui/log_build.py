...
def build_log_card_section(
    *,
    ...
    # Кнопка не выводится в интерфейс: она вела в GitHub Discussions
    # исходного проекта. Объект остаётся, потому что на него ссылаются
    # обновление переводов и включение/выключение по ходу проверки —
    # без родителя виджет просто не показывается.
