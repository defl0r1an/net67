...
class WindowsDefenderManager:
    ...
    def _run_reg_command(self, command: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(command, shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
    ...
    def disable_defender(self) -> Tuple[bool, int]:
        ...
        stop_result = subprocess.run('sc stop WinDefend', shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
        ...
        disable_result = subprocess.run('sc config WinDefend start=disabled', shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
    ...
    def enable_defender(self) -> Tuple[bool, int]:
        ...
        for i, cmd in enumerate(commands):
            ...
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
            if result.returncode == 0:
                success_count += 1
                
        # Включаем автозапуск службы
        enable_result = subprocess.run('sc config WinDefend start=auto', shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
        ...
        start_result = subprocess.run('sc start WinDefend', shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
    ...
    def get_defender_status(self) -> str:
        ...
        try:
            ...
            result = subprocess.run('sc query WinDefend', shell=True, capture_output=True, text=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
