...
class MaxBlockerManager:
    ...
    def _refresh_explorer_policies(self):
        ...
        try:
            ...
            subprocess.run(['gpupdate', '/force'], capture_output=True, timeout=5, creationflags=_NO_WINDOW)
            
            # Способ 2: Перезапуск Explorer (более агрессивный метод)
            # Закомментировано, так как может быть слишком навязчиво
            # subprocess.run(['taskkill', '/F', '/IM', 'explorer.exe'], capture_output=True, creationflags=_NO_WINDOW)
            # subprocess.run(['start', 'explorer.exe'], shell=True, capture_output=True, creationflags=_NO_WINDOW)
    ...
    def block_max_in_firewall(self) -> bool:
        ...
        try:
            ...
            for rule_cmd in rules:
                result = subprocess.run(rule_cmd, capture_output=True, text=True, shell=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
    ...
    def unblock_max_in_firewall(self) -> bool:
        ...
        try:
            ...
            for rule_cmd in rules:
                subprocess.run(rule_cmd, capture_output=True, text=True, shell=True, encoding='cp866', errors='replace', creationflags=_NO_WINDOW)
