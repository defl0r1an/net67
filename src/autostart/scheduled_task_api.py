...
net67.exe собран с manifest requireAdministrator, поэтому ярлык в папке
...
AUTOSTART_TASK_NAME = "net67 Autostart"

#: Имя задачи из исходного проекта. Нужно только для удаления: если
#: пользователь ставил net67 раньше, его задача осталась бы в
#: планировщике и запускала чужой exe параллельно с net67.
LEGACY_AUTOSTART_TASK_NAMES = ("ZapretGUI Autostart",)
...
def _register_autostart_task(exe_path: str, task_name: str) -> None:
    ...
    task_def.RegistrationInfo.Author = "net67"
    task_def.RegistrationInfo.Description = (
        "Автозапуск net67 в трее при входе в Windows"
    )
