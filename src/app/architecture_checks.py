...
def check_runtime_ui_bridge_is_feature_neutral() -> list[Problem]:
    ...
    return _scan_lines(
        [path],
        re.compile(
            r"\b(?:dns|hosts|presets|preset_|WindowUiSession|"
...
def check_window_state_sync_is_window_only() -> list[Problem]:
    ...
    return _scan_lines(
        [path],
        re.compile(
            r"\b(?:app_runtime\.features|features\.|load_background_preset|"
            r"load_mica_enabled|apply_aero_effect|apply_window_background)\b"
        ),
...
def check_no_feature_internals_from_external(files: list[Path]) -> list[Problem]:
    ...
    return _scan_lines(
        ...
        re.compile(
            ...
            r"profile_feature|dns_feature|hosts_feature|logs_feature|telegram_proxy_feature))"
        ),
...
def check_settings_json_is_single_app_storage(files: list[Path]) -> list[Problem]:
    ...
    app_storage_scopes = [
        path
        for path in files
        if _under(path, "src/hosts/", "src/settings/")
    ]
    problems.extend(
        _scan_lines(
            app_storage_scopes,
