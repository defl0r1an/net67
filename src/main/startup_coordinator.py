...
class StartupCoordinator:
    ...
    def _run_step(self, task_name: str, label: str, func, *, error_status=None) -> bool:
        ...
        try:
            ...
            # Без traceback по такому сообщению место падения не найти:
            # SystemError от PyQt («QMetaObject.Connection returned a result
            # with an exception set») вообще не называет ни файл, ни строку —
            # он лишь означает, что раньше упал какой-то Qt-слот.
            import traceback as _traceback

            log(_traceback.format_exc(), "❌ ERROR")
