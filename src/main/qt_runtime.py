...
def ensure_qt_runtime() -> QApplication:
    ...
    try:
        from shell.theme import apply_application_font
...
def application_bootstrap() -> QApplication:
    ...
    try:
        ...
        if not accent_hex:
            # Без этой ветки акцент оставался бирюзовым: это умолчание
            # самого qfluentwidgets (#009faa), а не выбор приложения.
            # Фирменный акцент при этом не применялся вообще.
            from branding import PALETTE_LIGHT

            accent_hex = PALETTE_LIGHT.accent
        if accent_hex:
