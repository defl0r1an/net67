...
_MANAGED_HOSTS_BEGIN = "# >>> net67:hosts managed begin >>>"
_MANAGED_HOSTS_END = "# <<< net67:hosts managed end <<<"
_MANAGED_HOSTS_NOTICE = "# Generated by net67. Do not edit this block manually."

# Маркеры прежней версии приложения. Записываем всегда новые, но при чтении
# и очистке обязаны узнавать и старые: иначе блок, добавленный до
# переименования, останется в системном hosts навсегда — найти и удалить
# его будет уже нечем.
_LEGACY_MANAGED_HOSTS_BEGIN = "# >>> zapretgui:hosts managed begin >>>"
_LEGACY_MANAGED_HOSTS_END = "# <<< zapretgui:hosts managed end <<<"


def _is_managed_block_begin(stripped: str) -> bool:
    return stripped in (_MANAGED_HOSTS_BEGIN, _LEGACY_MANAGED_HOSTS_BEGIN)


def _is_managed_block_end(stripped: str) -> bool:
    return stripped in (_MANAGED_HOSTS_END, _LEGACY_MANAGED_HOSTS_END)


def _get_hosts_bootstrap_signature() -> str:
    return _HOSTS_BOOTSTRAP_SIGNATURE_VERSION
...
def _remove_managed_hosts_block(lines: list[str]) -> tuple[list[str], int]:
    """Удаляет управляемый блок из hosts и возвращает строки без него."""
    new_lines: list[str] = []
    removed_entries = 0
    inside_block = False
    ...
    for line in lines:
        stripped = line.strip()
        if _is_managed_block_begin(stripped):
            inside_block = True
            continue
        if inside_block:
            if stripped and not stripped.startswith("#"):
                removed_entries += 1
            if _is_managed_block_end(stripped):
                inside_block = False
            continue
...
def _iter_managed_hosts_block_rows(lines: list[str]) -> list[tuple[str, str]]:
    """Возвращает строки (domain, ip), которые лежат внутри блока net67."""
...
def _insert_managed_hosts_block(new_lines: list[str], rows: list[tuple[str, str]], insert_at: int | None = None) -> None:
    """Добавляет управляемый блок перед обычными строками hosts."""
    if not rows:
        return
...
class HostsManager:
        ...
        try:
            ...
            for line in lines:
                ...
                if not line_stripped:
                    new_lines.append(line)
                    continue
                rewritten_line, line_removed_github = _rewrite_hosts_bootstrap_line(
                    line=line,
    ...
    def get_active_domains_map(self) -> dict[str, str]:
        """Возвращает {domain: ip} из блока net67 в системном hosts."""
    ...
    def apply_domain_ip_rows(self, domain_ip_rows: list[tuple[str, str]]) -> bool:
        """Применяет домены в системный hosts через один управляемый управляемый блок."""
        ...
        try:
            ...
            # Ничего не добавляем — просто очищаем управляемый блок
            if not desired_rows:
    ...
    def add_adobe_domains(self) -> bool:
        ...
        try:
            ...
            new_lines.append('# Generated by net67\n')
    ...
    def clear_hosts_file(self) -> bool:
        """Удаляет только управляемый блок из системного hosts."""
        log("🗑️ Очистка блока net67 в hosts", "DEBUG")
        ...
        try:
            ...
            if not safe_write_hosts_file("".join(new_lines)):
                log("Не удалось записать файл hosts после очистки блока net67")
                return False
            
            self.set_status(f"Блок net67 очищен: удалено {removed_count} записей")
            invalidate_hosts_file_cache()
            log(f"✅ Блок net67 в hosts очищен: удалено {removed_count} записей", "DEBUG")
            return True
