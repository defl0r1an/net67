...
def _get_app_root() -> Path:
    ...
    Файл лежит в `<корень>/src/hosts/`, поэтому корень — parents[2].
    """
    if PACKAGED_RUNTIME:
...
def _get_hosts_catalog_candidates() -> list[Path]:
    """Каталог лежит в `json/` рядом с приложением — и в сборке, и в репозитории.

    Раньше вне сборки путь вёл в соседний приватный репозиторий автора
    исходного проекта (его приватный репозиторий ресурсов). Его у net67
    нет и не будет, так что ветка просто никогда не срабатывала.
    """
    json_root = _get_app_root() / "json"
    return [
...
QUICK_SERVICES = [
    ...
    # SoundCloud здесь был и убран. Подмена адресов в hosts спасает
    # только от блокировки на уровне DNS — когда провайдер отдаёт
    # неверный адрес. Замер показал, что это не наш случай: nslookup
    # через 1.1.1.1 отдаёт настоящие адреса CloudFront (52.84.150.x), а
    # сайт всё равно не открывается. Значит, режут после разрешения
    # имени, и правильный адрес в hosts ничего не изменит.
    #
    # Домены переехали в lists/soundcloud.txt — там их может взять
    # движок winws, который работает как раз с таким видом блокировки.
]
