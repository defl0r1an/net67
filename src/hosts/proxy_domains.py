...
QUICK_SERVICES = [
    ...
    # Порядок этого списка задаёт порядок в интерфейсе: сначала идут
    # перечисленные здесь, потом всё остальное из каталога. Поэтому
    # новая запись ставится в конец — просили «добавить вниз».
