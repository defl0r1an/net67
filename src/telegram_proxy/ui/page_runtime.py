...
def build_relay_result_plan(
    *,
    ...
    if http_ok and zapret_running:
        return TelegramProxyRelayResultPlan(
            status_text=f"{base} — Relay: стратегия net67 ломает TLS",
            show_warning=True,
            warning_title="Стратегия net67 ломает Telegram прокси",
            warning_content=(
                "Что происходит: IP relay (149.154.167.220) доступен, "
                "но текущая стратегия net67 применяет desync к TLS "
                "и ломает подключение прокси.\n"
                "Что делать: смените стратегию net67 на другую, "
                "или выключите net67 и перезапустите прокси."
            ),
        )
    if http_ok and not zapret_running:
        return TelegramProxyRelayResultPlan(
            ...
            warning_content=(
                ...
                "Что делать: если net67 только что выключен — "
    ...
    if zapret_running:
        return TelegramProxyRelayResultPlan(
            status_text=f"{base} — Relay: недоступен, net67 запущен",
            show_warning=True,
            warning_title="Relay недоступен — возможно мешает net67",
            warning_content=(
                "Что происходит: relay (149.154.167.220) не отвечает "
                "ни по HTTP, ни по TLS. net67 запущен.\n"
