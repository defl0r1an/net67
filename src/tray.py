...
class SystemTrayManager:
    ...
    def __init__(self, window_port, icon_path, app_version, *, tray_feature):
        ...
        self._class_name = f"{APP_SLUG}TrayWindow_{os.getpid()}_{id(self):x}"
    ...
    def _build_notify_icon_data(self, flags: int) -> NOTIFYICONDATAW:
        ...
        data.szTip = _truncate_text(f"{APP_NAME} v{self.app_version}", 128)
        return data
    ...
    def _ensure_qt_menu(self) -> QMenu:
        ...
        # Подменю «Эффект акрилика окна» удалено вместе с ползунком на
        # странице оформления. Оно к тому же падало: обработчик собирал
        # AppearanceSettingsSaveWorker без обязательного request_id.
        menu.addSeparator()
        ...
        # Пункт «Консоль» убран: отладочное окно в рабочем приложении
        # менеджеру не нужно.
        ...
        if not self._tray_hint_shown_this_session:
            try:
                self.show_notification(
                    f"{APP_NAME} продолжает работать",
