...
DARK = ShellPalette(
    window="#101010",
    # Карточка чуть светлее, чем была.
    #
    # При #202020 её контраст к фону страницы #171717 составлял 1.10 —
    # на глаз карточка не отличалась от фона, и увидеть её край можно
    # было только наведя курсор. Стало 1.16, и вместе с обводкой этого
    # хватает, чтобы блок читался сам по себе.
    surface="#242424",
