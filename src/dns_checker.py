...
class DNSChecker:
    ...
    def _analyze_results(self, results: Dict, log_callback=None, should_stop=None):
        ...
        if results['summary']['dns_poisoning_detected']:
            ...
            self._log("Обнаружена DNS подмена! Смените DNS или используйте net67.", log_callback, should_stop)
            ...
            self._log("Если есть проблемы с сайтами - используйте net67 для обхода DPI.", log_callback, should_stop)
        else:
            self._log("✅ РЕЗУЛЬТАТ:", log_callback, should_stop)
            self._log("DNS работает корректно. Если сайты недоступны - используйте net67.", log_callback, should_stop)
