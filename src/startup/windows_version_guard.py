...
# Раньше вела на пост в Telegram-канале автора исходного проекта.
# Пустая строка = ссылка не показывается.
CONSOLE_VERSION_URL = ""
WINDOWS_VERSION_ERROR_TITLE = f"{APP_NAME} — неподдерживаемая Windows"
...
def _unsupported_message(os_name: str) -> str:
    parts = [
        ...
        f"{APP_NAME} требует Windows 10 1809 или новее "
        f"(build {MIN_WINDOWS_10_1809_BUILD}+).\n\n",
    ]

    # Абзац про консольную версию имеет смысл только при заданной ссылке.
    # Раньше она вела на канал автора; с пустым значением остался бы
    # заголовок «доступна консольная версия:» и пустая строка под ним.
    if CONSOLE_VERSION_URL:
        parts.append(
