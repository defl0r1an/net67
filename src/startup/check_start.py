    ...
    try:
        ...
        for pid, process_name in iter_process_records_winapi():
            ...
            err = (
                ...
                "mitmproxy использует тот же драйвер WinDivert, что и net67.\n"
                "Одновременная работа этих программ невозможна.\n\n"
                "Пожалуйста, завершите все процессы mitmproxy и перезапустите net67."
            )
            try:
                from log.log import log
...
def contains_special_chars(path: str) -> bool:
    """True, если путь действительно ломает запуск.

    Раньше здесь отвергались пробелы и всё, кроме букв, цифр и ``_ . : \\ /``.
    Это наследство исходного проекта, и оно неверно:

    * winws и оркестратор запускаются как ``subprocess.Popen(список)`` —
      Python сам расставляет кавычки, пробелы и дефисы безопасны;
    * служба Windows создаётся через CreateServiceW, а путь с пробелом
      берётся в кавычки в ``create_zapret_service``.

    Из-за старой проверки net67 ругался на обычную папку
    ``zapret-21.1.1.4`` (дефисы) и ругался бы на штатную установку в
    ``C:\\Program Files\\net67`` (пробел).

    Осталось единственное, что не чинится кавычками: сама кавычка внутри
    пути. В именах файлов Windows её и так не допускает, так что проверка
    остаётся страховкой от подставленного пути.
    """
    return '"' in str(path or "")
...
def check_path_for_special_chars():
    ...
    if contains_special_chars(path):
        error_message = (
            ...
            "В пути есть кавычка — командная строка для winws соберётся неверно.\n"
            "Переместите программу в папку без кавычек в имени и запустите снова."
        )
        try:
            from log.log import log
        ...
        notifications.append(
            advisory_notification(
                ...
                content=(
                    ...
                    "(например, C:\\net67) и запустить программу оттуда.\n\n"
...
def check_goodbyedpi() -> tuple[bool, str]:
    ...
    if still_exists:
        ...
        err = (
            ...
            "net67 v2 GUI несовместим с GoodbyeDPI.\n\n"
