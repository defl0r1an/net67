...
def update_logs_tabs_accessibility(pivot, *, current: object | None = None, language: str = "ru") -> None:
    ...
    set_control_accessibility(
        pivot,
        name=state,
        description="Выберите раздел страницы логов: Логи или Управление.",
    )
...
class LogsPage(BasePage):
    ...
    def _build_ui(self):
        ...
        # Вкладка «Поддержка» вела в GitHub Discussions исходного проекта:
        # собирала ZIP логов и открывала там обращение. К net67 отношения
        # не имеет. Страница остаётся в стеке под индексом 1, чтобы не
        # сдвигать индексы «Управления», но открыть её больше нечем.
        self.tabs_pivot.addItem(
            routeKey="manage",
