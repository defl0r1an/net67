...
class Logger:
    ...
    def log(self, message, level="INFO", component=None):
        # Разбор один и здесь. В поле уровня за годы накопилось всё
        # подряд: значки («❌ ERROR»), источники («📱 TG», «POOL»),
        # синонимы («SUCCESS»). Править двести мест вызова бессмысленно —
        # следующая правка добавит двести первое, — поэтому строка
        # приводится к виду на входе. Подробности в log/normalize.py.
        from log.normalize import normalize

        message, level, component = normalize(message, level, component)
...
#: Признаки строки-замера. Такие сообщения уходят в DEBUG.
#:
#: Проверяем по началу строки, а не по вхождению: слово «замер» может
#: встретиться и в обычном сообщении, которое человеку читать надо.
#: Имена метрик запуска идут слитно — «StartupQtRuntimeQApplication», —
#: поэтому для них хватает одной приставки.
_MEASUREMENT_PREFIXES = ("UiMetric:", "Startup")


def _is_measurement(message) -> bool:
    text = str(message or "").lstrip()
    return text.startswith(_MEASUREMENT_PREFIXES)
