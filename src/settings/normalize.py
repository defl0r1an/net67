...
def _as_panel_side(value, default: str) -> str:
    """Сторона панели расширенного режима.

    Проверку держим здесь, а не в ui: настройки должны нормализоваться
    без импорта интерфейса, иначе их нельзя прочитать в фоновом потоке.
    """
    key = str(value or "").strip().lower()
    return key if key in ("left", "right", "top") else str(default)
...
def normalize_ui_state(data: object) -> dict[str, Any]:
    ...
    return {
        ...
        "advanced_mode": as_bool(raw.get("advanced_mode"), defaults["advanced_mode"]),
        "wizard_completed": as_bool(raw.get("wizard_completed"), defaults["wizard_completed"]),
        "wizard_services": sorted(
            {
                str(item)
                for item in (raw.get("wizard_services") or [])
                if isinstance(item, str) and str(item).strip()
            }
        ),
        "advanced_panel_side": _as_panel_side(
            raw.get("advanced_panel_side"), defaults["advanced_panel_side"]
        ),
        "autotune_enabled": as_bool(raw.get("autotune_enabled"), defaults["autotune_enabled"]),
        "provider_key": as_clean_str(raw.get("provider_key"), defaults["provider_key"]),
        "hosts_defaults_version": as_int(
            raw.get("hosts_defaults_version"),
            # Старый булев флаг означал «применена версия 1».
            1 if as_bool(raw.get("hosts_defaults_applied"), False) else defaults["hosts_defaults_version"],
        ),
    }
