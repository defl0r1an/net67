...
def at_config_path_from(launch_arg, work_dir) -> Path:
    """Путь к @config из аргумента запуска.

    Аргумент намеренно относительный: winws — cygwin-бинарник и не
    разбирает «@C:\\Program Files\\...». Он запускается с cwd=work_dir,
    поэтому и тест разрешает путь так же.
    """
    text = str(launch_arg or "")
    assert text.startswith("@"), f"аргумент не @config: {text!r}"
    path = Path(text[1:])
    return path if path.is_absolute() else Path(work_dir) / path
...
class Winws2LaunchPresetValidationTests(unittest.TestCase):
    ...
    def test_winws2_compile_accepts_explicit_lists_paths_for_at_launch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            at_config_path = at_config_path_from(artifact.launch_args[0], runner.work_dir)
    ...
    def test_winws2_compile_launches_safe_at_config_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(at_config_path_from(rebuilt.launch_args[0], runner.work_dir).exists())
    ...
    def test_winws2_dry_run_artifact_stays_inside_at_config(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dry_run_config = at_config_path_from(
                dry_run_artifact.launch_args[0], runner.work_dir
    ...
    def test_winws1_compile_resolves_bare_list_paths_for_launch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(artifact.validation_ok, artifact.validation_report)
            self.assertEqual(len(artifact.launch_args), 1)
            at_config_text = at_config_path_from(
                artifact.launch_args[0], runner.work_dir
            ).read_text(encoding="utf-8")
