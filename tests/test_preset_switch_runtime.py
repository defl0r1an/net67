...
def at_config_path_from(launch_arg, work_dir) -> Path:
    ...
    Аргумент относительный: winws — cygwin-бинарник и не разбирает
    «@C:\\Program Files\\...». Он стартует с cwd=work_dir, и тест
    разрешает путь так же.
    """
...
class Winws2PresetSwitchTests(unittest.TestCase):
    ...
    def test_winws2_compile_rebuilds_at_config_when_same_size_content_changes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            first_config = at_config_path_from(first_artifact.launch_args[0], runner.work_dir).read_text(encoding="utf-8")

            preset_path.write_text("--wf-udp-out=443\n", encoding="utf-8")
            os.utime(preset_path, ns=(fixed_ns, fixed_ns))
            second_artifact = runner._compile_preset_artifact(str(preset_path))
            second_config = at_config_path_from(second_artifact.launch_args[0], runner.work_dir).read_text(encoding="utf-8")
    ...
    def test_winws2_fast_switch_starts_new_process_before_stopping_old(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            handoff_config = at_config_path_from(spawned_artifact.launch_args[0], runner.work_dir).read_text(encoding="utf-8")
