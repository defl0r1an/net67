...
class PresetProfileAsyncArchitectureTests(unittest.TestCase):
    ...
    def test_slow_navigation_pages_log_local_timing_stages(self) -> None:
        ...
        self.assertIn("appearance_ui.accent_section.build", appearance_source)
    ...
    def test_appearance_page_builds_all_sections_in_initial_pass(self) -> None:
        """Отложенной достройки больше нет — и не должно появиться снова.

        Раньше страница строила нижние секции вторым проходом по таймеру,
        и на слабых машинах человек успевал увидеть полупустое окно.
        Секции «Праздничное оформление», «Эффект акрилика» и
        «Производительность» удалены, остаток строится за один проход.
        """
        build_source = inspect.getsource(AppearancePage._build_ui)

        self.assertFalse(hasattr(AppearancePage, "_ensure_lower_sections_built"))
        self.assertFalse(hasattr(AppearancePage, "_schedule_lower_sections_build"))
        self.assertIn("appearance_ui.accent_section.build", build_source)
        self.assertNotIn("QTimer.singleShot", build_source)
