...
class EarlyWindowsVersionGuardTests(unittest.TestCase):
    def test_rejects_windows_7_8_and_81_with_console_version_guidance(self) -> None:
        ...
        for version, os_name in cases:
            with self.subTest(os_name=os_name):
                ...
                self.assertIn(APP_NAME, result.message)
                # Ссылка на консольную версию вела на канал автора и
                # удалена: абзац показывается только если она задана.
                self.assertNotIn("t.me", result.message)
    ...
    def test_guard_shows_plain_windows_message_and_exits_before_qt(self) -> None:
        ...
        self.assertEqual(title, f"{APP_NAME} — неподдерживаемая Windows")
