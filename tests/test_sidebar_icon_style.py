...
class SidebarIconStyleNavigationTests(unittest.TestCase):
    ...
    def test_windows11_fluent_style_falls_back_without_svg_pack(self) -> None:
        """Набор SVG-иконок Windows 11 в поставку net67 не входит.

        Переключатель стиля иконок из интерфейса убран, поэтому стиль
        всегда «standard». Но значение может остаться в настройках от
        прошлых версий, и запрос несуществующего набора обязан отдать
        рабочую иконку, а не пустую и не исключение.
        """
        ...
        for page_name in (PageName.ZAPRET2_USER_PRESETS, PageName.LOGS):
    ...
    def test_apply_sidebar_icon_style_updates_existing_items(self) -> None:
        ...
        # Иконка ставится каждому пункту ровно один раз, даже когда набор
        # SVG отсутствует и работает запасной вариант.
        self.assertEqual(len(preset_item.icons), 1)
        self.assertIsNotNone(preset_item.icons[0])
        self.assertEqual(len(logs_item.icons), 1)
        self.assertIsNotNone(logs_item.icons[0])
        self.assertIsNotNone(session.default_nav_icon)


class SidebarIconStyleAppearancePageTests(unittest.TestCase):
    def test_the_appearance_page_is_gone(self) -> None:
        """Здесь проверялось, что раздел «Иконки меню» убран со страницы.

        Страницы оформления больше нет целиком. Стиль иконок при этом
        остаётся в настройках и читается при сборке меню — проверки
        ...
        self.assertFalse((root / "src" / "ui" / "pages" / "appearance_page.py").exists())
