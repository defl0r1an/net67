...
# Раньше шаблон профилей и списки лежали в соседнем приватном
# репозитории автора исходного проекта. У net67 он один: шаблон переехал
# в src/profile/templates, списки — в lists/base рядом с приложением.
ALL_PROFILES_PATH = PUBLIC_ROOT / "src" / "profile" / "templates" / "all_profiles.txt"
LISTS_ROOT = PUBLIC_ROOT / "lists" / "base"
...
ACCEPTED_LEGACY_CLOUDFLARE_TCP_PROFILES = {
    ("Стандартный 1.txt", "Cloudflare legacy TCP"),
    ("Стандартный 1.txt", "Cloudflare IPv6 legacy TCP"),
    ("Стандартный 1.txt", "Cloudflare alt TCP"),
}
...
class BuiltinProfileCatalogTests(unittest.TestCase):
    ...
    def test_amazon_as16509_ranges_are_shipped_and_default_v1_uses_ipset(self) -> None:
        ...
        preset_path = PUBLIC_ROOT / "src" / "presets" / "builtin" / "winws2" / "Стандартный 1.txt"
        preset = parse_preset_text(
        ...
        ipset_path = LISTS_ROOT / "ipset-amazon.txt"
    ...
    def test_githubusercontent_has_own_hostlist_file(self) -> None:
        github_hosts = [
            line.strip()
            for line in (LISTS_ROOT / "github.txt").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        githubusercontent_hosts = [
            line.strip()
            for line in (LISTS_ROOT / "githubusercontent.txt")
    ...
    def test_real_default_v5_profile_list_recognizes_discord_profiles_from_catalog(self) -> None:
        ...
        service = ProfilePresetService(
            SimpleNamespace(
                ...
                _app_paths=AppPaths(user_root=PUBLIC_ROOT / "src", local_root=PUBLIC_ROOT / "src"),
            ),
            "zapret2_mode",
        )
    ...
    def test_vencord_profile_uses_dedicated_hostlist(self) -> None:
        ...
        vencord_entries = _list_entries(LISTS_ROOT / "vencord.txt")
        discord_entries = _list_entries(LISTS_ROOT / "discord.txt")
    ...
    def test_hetzner_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-hetzner.txt"
        entries = [
            line.strip()
    ...
    def test_cloudflare_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-cloudflare.txt"
        entries = [
            line.strip()
            for line in list_path.read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
    ...
    def test_digitalocean_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-digitalocean.txt"
        entries = [
            line.strip()
            for line in list_path.read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
    ...
    def test_datacamp_profiles_have_shipped_ipset(self) -> None:
        ...
        builtin_path = PUBLIC_ROOT / "src" / "presets" / "builtin" / "winws2" / "Стандартный 1.txt"
        ...
        list_path = LISTS_ROOT / "ipset-datacamp.txt"
    ...
    def test_ovh_profiles_have_updated_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-ovh.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_roblox_profiles_have_updated_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-roblox.txt"
        entries = [
            line.strip()
            for line in list_path.read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
    ...
    def test_novoserve_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-novoserve.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_worldstream_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-worldstream.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_fastly_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-fastly.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_akamai_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-akamai.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_default_v1_uses_single_facebook_ipset_profile(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-facebook.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_frantech_solutions_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-frantech.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_railway_profiles_have_shipped_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-railway.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
    ...
    def test_google_profiles_have_shipped_ipset_and_default_v1_pass(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-google.txt"
        entries = [
            line.strip()
            for line in list_path.read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
    ...
    def test_google_cloud_profiles_have_shipped_usa_google_ipset(self) -> None:
        ...
        list_path = LISTS_ROOT / "ipset-usa-google.txt"
        raw_lines = list_path.read_text(encoding="utf-8").splitlines()
        entries = [
            line.strip()
            for line in raw_lines
