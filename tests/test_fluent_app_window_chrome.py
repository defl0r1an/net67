...
class SearchRemovalTests(unittest.TestCase):
    """Строка поиска убрана из полосы заголовка.

    Искать не в чем: четыре раздела, в каждом три-четыре страницы. Места
    же она занимала больше всех остальных элементов заголовка вместе
    взятых.

    Сам поиск оставлен рабочим — на нём держатся подсказки, переход по
    результату и обход страниц, — поэтому проверки его размещения ниже
    не удалены, а пропускаются по флагу. Вернут флаг — вернутся и они.
    """

    def test_search_is_not_placed_in_the_title_bar(self) -> None:
        from ui.navigation.search import SEARCH_IN_TITLEBAR

        self.assertFalse(SEARCH_IN_TITLEBAR)

    def test_attach_is_a_no_op_while_it_is_off(self) -> None:
        """Иначе поиск лез бы в раскладку и растаскивал кнопки окна."""
        import inspect

        from ui.navigation import search

        source = inspect.getsource(search.attach_sidebar_search_to_titlebar)

        self.assertIn("if not SEARCH_IN_TITLEBAR:", source)
        self.assertLess(source.index("SEARCH_IN_TITLEBAR"), source.index("get_window_ui_session"))
...
class FluentAppWindowChromeTests(unittest.TestCase):
    ...
    def test_titlebar_search_has_screen_reader_text(self) -> None:
        ...
        self.assertEqual(search_widget.accessibleName(), "Глобальный поиск по net67")
        self.assertEqual(search_widget.property("screenReaderStateText"), "Глобальный поиск по net67")
        self.assertIn("страницу, preset или profile", search_widget.accessibleDescription())
        self.assertEqual(search_widget._search.accessibleName(), "Глобальный поиск по net67")
        self.assertEqual(search_widget._search.property("screenReaderStateText"), "Глобальный поиск по net67")
    ...
    @unittest.skipUnless(
        __import__("ui.navigation.search", fromlist=["SEARCH_IN_TITLEBAR"]).SEARCH_IN_TITLEBAR,
        "строка поиска убрана из полосы заголовка, см. SearchRemovalTests",
    )
