...
# Файл фикстуры попадает под правило *.log в .gitignore и поэтому
# отсутствует в свежей копии репозитория. Без него тесты не могут
# работать, но это нехватка данных, а не поломка разбора логов —
# честнее пропустить, чем выдавать десять ошибок подряд.
pytestmark = pytest.mark.skipif(
    not FIXTURE.is_file(),
    reason=(
        f"нет файла фикстуры {FIXTURE.name}: он исключён правилом *.log "
        "в .gitignore и не попал в репозиторий"
    ),
)
