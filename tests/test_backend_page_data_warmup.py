...
class BackendPageDataWarmupTests(unittest.TestCase):
    def test_installer_warms_backend_caches_without_page_host(self) -> None:
        ...
        # Отсрочек стало одна. Вторая, на 18 секундах, была нужна ради
        # прогрева страницы доната: та ходила в сеть, и её отодвигали
        # подальше от старта. Страницы нет — нет и отсрочки.
        self.assertEqual(delays, [8000])
        self.assertEqual(
            queued_tasks,
            [
    ...
    def test_the_donation_page_is_gone_for_good(self) -> None:
        """Здесь проверялся прогрев страницы доната.

        Страницы нет: подписок в net67 не бывает, а пакет donater вёл
        переписку с чужим Telegram-ботом и хранил привязку устройства.
        Проверку заменяем на противоположную — чтобы пакет не вернулся
        случайным откатом.
        """
        import importlib

        for module_name in ("donater", "donater.ui.page", "app.feature_facades.premium"):
