...
class UserPresetsAccessibilityTests(unittest.TestCase):
    ...
    def _build_widgets(self, *, on_preset_list_action=None):
        ...
        widgets = build_user_presets_page_shell(
            ...
            # Кнопки «Получить конфиги» на странице больше нет: она
            # вела в чужой репозиторий, а net67 — внутренний
            # инструмент. Аргумент остался в тесте и ронял его не на
            # ошибке, а на своей устарелости.
    ...
    def test_info_dialog_has_no_link_to_the_upstream_site(self) -> None:
        """Кнопка «Открыть сайт с пресетами» убрана.

        Раньше проверка стерегла обратное — что кнопка есть и открывает
        сайт. Кнопка вела на сайт исходного проекта, стороннего по
        отношению к net67, и в корпоративной сборке отправляла
        руководителей туда, куда не надо. Это третья такая кнопка: до неё
        так же убрали «Заявку на GitHub» и «Получить конфиги».

        Проверка не ослаблена, а перевёрнута: стережём отсутствие.
        """
        page = UserPresetsPageBase.__new__(UserPresetsPageBase)
        page._config = SimpleNamespace(tr_prefix="page.user_presets")
        page._tr = lambda _key, default, **kwargs: default.format(**kwargs) if kwargs else default
    ...
    def test_user_preset_help_text_explains_preset_file_and_wiki(self) -> None:
        for prefix in ("page.winws1_user_presets", "page.winws2_user_presets"):
            with self.subTest(prefix=prefix):
                ...
                self.assertIn("preset-zapret2.txt", body)
    ...
    def _assert_accessibility(self, widgets) -> None:
        expected = {
            # Кнопки «Получить конфиги» и её заголовка на странице
            # больше нет: они вели в GitHub Discussions чужого проекта.
            # Ожидания остались в тесте и роняли его не на ошибке, а на
            # своей устарелости.
