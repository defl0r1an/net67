...
class WinDivertServiceRecoveryTests(unittest.TestCase):
    ...
    def test_spawn_readiness_waits_for_delete_pending_monkey(self) -> None:
        """Драйвер в DELETE_PENDING нужно дождаться, а не стартовать поверх.

        Раньше здесь стояло sleep.assert_not_called(): проверка выходила
        на первой же итерации и разрешала запуск немедленно. Так и
        появлялся симптом «включил, выключил, включил — не работает»:
        winws стартовал поверх ещё живого драйвера, получал бесполезный
        дескриптор, и трафик не фильтровался. В логе при этом всё чисто —
        процесс запущен, PID выдан.

        DELETE_PENDING держится пару секунд, поэтому его пережидают.
        """
        from winws_runtime.runtime import system_ops
        ...
        def stale_then_clean():
            stale_calls["n"] += 1
            return ["Monkey"] if stale_calls["n"] <= 2 else []
        ...
        self.assertTrue(result.ready)
        self.assertEqual(result.stage, "network_open", "дождались чистого драйвера")
        self.assertGreaterEqual(sleep.call_count, 2, "ожидания не было")

    def test_spawn_readiness_starts_anyway_if_driver_never_unloads(self) -> None:
        """Отказать в запуске нельзя — кнопка «Включить» перестанет работать."""
        from winws_runtime.runtime import system_ops

        blocked_probe = system_ops.WinDivertRuntimeProbeResult(
            installed=True,
            ready=False,
            error_code=None,
            stage="network_open",
        ...
        with (
            patch.object(system_ops, "probe_windivert_state_runtime", return_value=blocked_probe),
            patch.object(
                system_ops,
                "find_stale_windivert_delete_pending_services_runtime",
                return_value=["Monkey"],
            ...
            patch.object(system_ops.time, "sleep"),
            patch.object(system_ops, "STALE_DRIVER_UNLOAD_EXTRA_WAIT_SECONDS", 0.05),
            patch.object(system_ops, "log"),
        ):
            result = system_ops.wait_for_windivert_spawn_ready_runtime(
