...
class LogsAccessibilityTests(unittest.TestCase):
    ...
    def test_tabs_read_current_section_for_screen_reader(self) -> None:
        ...
        # Вкладка «Поддержка» удалена: она собирала обращение в адрес
        # автора исходного проекта.
        self.assertNotIn("send", page.tabs_pivot.items)
        self.assertEqual(page.tabs_pivot.accessibleName(), "Вкладки страницы логов, выбрано: Логи")
        self.assertIn("Логи или Управление", page.tabs_pivot.accessibleDescription())

        page.tabs_pivot.setCurrentItem("manage")

        self.assertEqual(page.tabs_pivot.accessibleName(), "Вкладки страницы логов, выбрано: Управление")
        self.assertEqual(
            page.tabs_pivot.property("screenReaderStateText"),
            "Вкладки страницы логов, выбрано: Управление",
        )
    ...
    def test_tabs_items_read_name_and_selection_for_screen_reader(self) -> None:
        ...
        self.assertEqual(
            page.tabs_pivot.items["manage"].accessibleName(),
            "Вкладки страницы логов: Управление, выбрано",
        )
