...
class ControlStatusDotPulseTests(unittest.TestCase):
    ...
    def test_apply_status_plan_sets_screen_reader_status_text(self) -> None:
        ...
        self.assertEqual(status_title.accessible_name, "Статус net67: Ошибка запуска")
        self.assertEqual(
            status_title.properties.get("screenReaderStateText"),
            "Статус net67: Ошибка запуска",
        )
        ...
        self.assertEqual(
            ...
            "Описание состояния net67: Не удалось запустить процесс обхода блокировок",
        )
        self.assertEqual(
    ...
    def test_apply_status_plan_sets_screen_reader_dot_state_text(self) -> None:
        ...
        self.assertEqual(status_dot.accessible_name, "Индикатор состояния net67: Ошибка запуска")
        self.assertEqual(
            status_dot.properties.get("screenReaderStateText"),
            "Индикатор состояния net67: Ошибка запуска",
        )
    ...
    def test_last_status_message_sets_screen_reader_label_context(self) -> None:
        ...
        self.assertEqual(
            ...
            "Последнее сообщение net67: Пресет успешно применён",
        )
        self.assertEqual(
    ...
    def test_loading_status_text_updates_screen_reader_state(self) -> None:
        ...
        self.assertEqual(label.accessibleName(), "Статус запуска net67: Запуск winws...")
        self.assertEqual(label.property("screenReaderStateText"), "Статус запуска net67: Запуск winws...")

        set_loading_status_accessibility(label, active=False, text="")

        self.assertEqual(label.accessibleName(), "Статус запуска net67: нет активного запуска")
        self.assertEqual(label.property("screenReaderStateText"), "Статус запуска net67: нет активного запуска")
