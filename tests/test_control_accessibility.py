...
class ControlAccessibilityTests(unittest.TestCase):
    ...
    def test_management_buttons_have_screen_reader_names_and_descriptions(self) -> None:
        ...
        self.assertEqual(start_btn.property("screenReaderStateText"), "Запустить net67")
        ...
        self.assertEqual(progress.accessibleName(), "Ход запуска net67: не выполняется")
        self.assertEqual(progress.property("screenReaderStateText"), "Ход запуска net67: не выполняется")
        self.assertIn("Показывает", progress.accessibleDescription())
        self.assertEqual(loading.accessibleName(), "Статус запуска net67: нет активного запуска")
        self.assertEqual(loading.property("screenReaderStateText"), "Статус запуска net67: нет активного запуска")
    ...
    def test_status_dot_has_initial_screen_reader_state(self) -> None:
        ...
        self.assertEqual(status_dot.accessibleName(), "Индикатор состояния net67: состояние пока не загружено")
        self.assertEqual(
            status_dot.property("screenReaderStateText"),
            "Индикатор состояния net67: состояние пока не загружено",
        )
    ...
    def test_status_section_has_initial_screen_reader_text(self) -> None:
        ...
        self.assertEqual(status_title.accessibleName(), "Статус net67: Проверка состояния")
        self.assertEqual(status_desc.accessibleName(), "Описание состояния net67: Определяем текущий статус")
    ...
    def test_last_status_message_card_has_initial_screen_reader_text(self) -> None:
        ...
        self.assertEqual(widgets.title_label.accessibleName(), "Раздел статуса net67: Последнее сообщение")
        self.assertEqual(widgets.message_label.accessibleName(), "Последнее сообщение net67: Пока нет новых сообщений")
